import { Addnote, Delnote, Comments, LikeDisLike } from "../actions/Action";

const InitialState = {
    PostDetails: [
        {
            Name: "Riya Roy",
            Email: "Riyaroy1001@gmail.com",
            Post: "Hey,there I am Riya studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 45,
            DL: 63,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Kousik Roy",
            Email: "kaushikroy1001@gmail.com",
            Post: "Hey,there I am kousik studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 18,
            DL: 10,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Kishan Roy",
            Email: "Kishanroy1001@gmail.com",
            Post: "Hey,there I am Kishan studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 87,
            DL: 67,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Aman Roy",
            Email: "Amanroy1001@gmail.com",
            Post: "Hey,there I am Aman studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 23,
            DL: 41,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Riya Roy",
            Email: "Riyaroy1001@gmail.com",
            Post: "Hey,there I am Riya studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 45,
            DL: 63,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Kousik Roy",
            Email: "kaushikroy1001@gmail.com",
            Post: "Hey,there I am kousik studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 18,
            DL: 10,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Kishan Roy",
            Email: "Kishanroy1001@gmail.com",
            Post: "Hey,there I am Kishan studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 87,
            DL: 67,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Aman Roy",
            Email: "Amanroy1001@gmail.com",
            Post: "Hey,there I am Aman studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 23,
            DL: 41,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Riya Roy",
            Email: "Riyaroy1001@gmail.com",
            Post: "Hey,there I am Riya studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 45,
            DL: 63,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Kousik Roy",
            Email: "kaushikroy1001@gmail.com",
            Post: "Hey,there I am kousik studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 18,
            DL: 10,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Kishan Roy",
            Email: "Kishanroy1001@gmail.com",
            Post: "Hey,there I am Kishan studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 87,
            DL: 67,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Aman Roy",
            Email: "Amanroy1001@gmail.com",
            Post: "Hey,there I am Aman studying in charusat university.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 23,
            DL: 41,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
        {
            Name: "Riya Roy",
            Email: "Riyaroy1001@gmail.com",
            Post: "Hey,there I am Riya studying in charusat university this one only.",
            comment: [
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
                {
                    name: "Kousik Roy",
                    email: "kaushikroy1001@gmail.com",
                    content: "Hello Riya Roy",
                    tm: 1643011821849,
                },
            ],
            L: 45,
            DL: 63,
            time: 1643011821849,
            ImgLink: "https://picsum.photos/450/200",

            Like: true,
            DisLike: true,

        },
    ],
};

export function Discussion(state = InitialState, action) {
    switch (action.type) {
        case Addnote:
            return {
                ...state,
                PostDetails: [action.newNote, ...state.PostDetails],
            };
            break;
        case Delnote:
            const filtered = state.PostDetails.filter((noteItem, index) => {
                return index !== action.id;
            });

            return {
                ...state,
                PostDetails: filtered,
            };

            break;
        case Comments:
            const { Name, Email, content: comm, value } = action.obj;
            const id = action.Id;
            const objnew = {
                name: Name,
                email: Email,
                content: comm,
                tm: value,
            };
            console.log(objnew);
            return {
                ...state,
                PostDetails: state.PostDetails.map((content, i) =>
                    i === id
                        ? { ...content, comment: [objnew, ...content.comment] }
                        : content
                ),
            };

            break;
        case LikeDisLike:
            var idval = action.Id;

            switch (action.operation) {
                case "L+":

                    let newOBJ = state.PostDetails.map((content, i) =>
                        i === idval
                            ? {
                                ...content,
                                L: action.Lvalue,
                                DL: action.DLvalue,
                                Like: action.setLiked,
                                DisLike: true,
                            }
                            : content
                    );
                    console.log(newOBJ)
                    return {
                        ...state,
                        PostDetails: newOBJ,
                    };
                    break;
                case "DL+":
                    let newOBJ2 = state.PostDetails.map((content, i) =>
                        i === idval
                            ? {
                                ...content,
                                L: action.Lvalue,
                                DL: action.DLvalue,
                                DisLike: action.setLiked,
                                Like: true,
                            }
                            : content
                    );
                    console.log(newOBJ2)
                    return {
                        ...state,
                        PostDetails: newOBJ2,
                    };
                    break;

                case "DL-&L+":
                    let newObj1 = state.PostDetails.map((content, i) =>
                        i === idval
                            ? {
                                ...content,
                                L: action.Lvalue,
                                DL: action.DLvalue,
                                Like: action.setLiked,
                                DisLike: true,
                            }
                            : content
                    );
                    console.log(newObj1)
                    return {
                        ...state,
                        PostDetails: newObj1,
                    };
                    break;
                case "DL+&L-":
                    let newObj2 = state.PostDetails.map((content, i) =>
                        i === idval
                            ? {
                                ...content,
                                L: action.Lvalue,
                                DL: action.DLvalue,
                                DisLike: action.setLiked,
                                Like: true,
                            }
                            : content
                    );
                    console.log(newObj2)
                    return {
                        ...state,
                        PostDetails: newObj2,
                    };
                    break;

                default:
                    return state;
                    break;
            }

        default:
            return state;
            break;
    }
}

export default Discussion;
